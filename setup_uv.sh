...
if [ "$NEW_ENV" = true ] ; then
    ...
    # Auto-detect system CUDA version to install matching PyTorch wheels
    if [ -x /usr/local/cuda/bin/nvcc ]; then
        ...
        SYSTEM_CUDA=$((nvidia-smi 2>/dev/null || true) | grep "CUDA Version" | sed 's/.*CUDA Version: *\([0-9]*\)\.\([0-9]*\).*/\1.\2/')
    else
        SYSTEM_CUDA=""
    fi
    ...
    if [ "$SYSTEM_CUDA_MAJOR" = "12" ] && [ "$SYSTEM_CUDA_MINOR" -ge 4 ] 2>/dev/null; then
        ...
        echo "[WARNING] Could not detect system CUDA version (got: '$SYSTEM_CUDA'). Defaulting to cu121."
        TORCH_INDEX="cu121"
    fi

    echo "[SETUP] System CUDA: $SYSTEM_CUDA -> using PyTorch index: $TORCH_INDEX"
    uv pip install torch==2.6.0 torchvision==0.21.0 torchaudio==2.6.0 --index-url https://download.pytorch.org/whl/$TORCH_INDEX
fi

# Get system information
WORKDIR=$(pwd)
PYTORCH_VERSION_FULL=$(python -c "import torch; print(torch.__version__)")
# Strip local version suffix (e.g. 2.4.0+cu118 -> 2.4.0) for version matching
PYTORCH_VERSION=$(echo $PYTORCH_VERSION_FULL | sed 's/+.*//')
...
case $PLATFORM in
    cuda)
        ...
        echo "[SYSTEM] PyTorch Version: $PYTORCH_VERSION_FULL, CUDA Version: $CUDA_VERSION, CUDA_HOME: $CUDA_HOME"
        ;;
    hip)
        ...
        echo "[SYSTEM] Installing PyTorch 2.4.1 for HIP ($PYTORCH_VERSION_FULL -> 2.4.1+rocm6.1)"
...
if [ "$BASIC" = true ] ; then
    uv pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh open3d xatlas pyvista pymeshfix igraph transformers==4.50.0 qwen-vl-utils 'accelerate>=0.26.0' ipdb
    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        if [ "$CUDA_VERSION" = "11.8" ] ; then
            case $PYTORCH_VERSION in
                ...
                2.6.0) uv pip install xformers==0.0.29.post1 --index-url https://download.pytorch.org/whl/cu118 ;;
            ...
            case $PYTORCH_VERSION in
                ...
                2.6.0) uv pip install xformers==0.0.29.post1 --index-url https://download.pytorch.org/whl/cu121 ;;
                *) echo "[XFORMERS] Unsupported PyTorch & CUDA version: $PYTORCH_VERSION & $CUDA_VERSION" ;;
            esac
        elif [ "$CUDA_VERSION" = "12.4" ] ; then
            case $PYTORCH_VERSION in
                2.4.0) uv pip install xformers==0.0.27.post2 ;;
                2.4.1) uv pip install xformers==0.0.28 --index-url https://download.pytorch.org/whl/cu124 ;;
                2.5.0) uv pip install xformers==0.0.28.post2 --index-url https://download.pytorch.org/whl/cu124 ;;
                2.6.0) uv pip install xformers==0.0.29.post3 --index-url https://download.pytorch.org/whl/cu124 ;;
                *) echo "[XFORMERS] Unsupported PyTorch & CUDA version: $PYTORCH_VERSION & $CUDA_VERSION" ;;
            esac
        else
        ...
        case $PYTORCH_VERSION in
            ...
            2.4.0)
                case $CUDA_MAJOR_VERSION in
                    11) uv pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu118.html;;
                    12) uv pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu121.html;;
                    *) echo "[KAOLIN] Unsupported CUDA version: $CUDA_MAJOR_VERSION" ;;
                esac ;;
...
if [ "$VOX2SEQ" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        uv pip install /tmp/extensions/vox2seq --no-build-isolation
    else
